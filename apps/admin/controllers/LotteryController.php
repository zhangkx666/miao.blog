...
class LotteryController extends BaseController
{
    ...
    public function threeAction()
    {
        ...
        // 查询历史记录数据量
        $limit = $_GET['limit'] ? (int)$_GET['limit'] : 60;

        // 延迟期数
        $offset = 0;

//         $bet_times = [0.5, 1, 2, 4, 8, 15, 30, 55, 100, 200, 400, 800]; // 4码
        // $bet_times = [1, 2, 5, 10, 22, 50, 100, 200, 500];  // 5码
        // $bet_times = [1, 2, 3, 4, 6, 8, 12, 17, 25, 36, 53]; // 3码
        // $bet_times = [5, 6, 7, 9, 11, 14, 18, 23, 28, 36, 45, 56, 70];  // 2码
        // $bet_times = [10, 25, 70];  // 6码
//         $bet_times = [0.2, 0.4, 0.8, 1.5, 3, 5.2, 9, 16, 28, 48, 82, 140, 240]; // 柳儿专用倍率
//        $bet_times = [1, 2, 4, 8, 15, 30, 55, 100, 200, 350, 600];
        $bet_times = [5, 10, 20, 25];
//        $bet_times = [1, 2, 4, 5];
//        $bet_times = [1, 1, 2, 3];
//        $bet_times = [1, 3, 0, 0];

//        $bet_times = [1, 2, 5, 21, 56, 150, 400, 100, 200, 350, 600]; // 6码

//        $bet_times = [2, 5, 10, 22, 48, 100, 210, 440, 920, 2000, 4200, 10000]; // 5码
//        $bet_times = [1, 2, 4, 8, 15, 30, 52, 90, 160, 280, 480, 820, 1400, 2400];
//        $bet_times = [0.2, 0.4, 0.8, 1.5, 3, 5.2, 9, 16, 28, 48, 82, 140, 240, 500, 1000, 2000];

//        $bet_times = [10, 22, 48, 100, 210, 440, 920, 2000, 4200]; // 5码
        // 6码
//        {% set _cost = [12, 48, 144, 396, 1068, 2868, 7668][_counter] %}
//        {% set _money = [7.2, 9.6, 9.6, 7.2, 7.2, 12, 12][_counter] %}
        ...
        $code_count = $_GET['code_count'] ? (int)$_GET['code_count'] : 4;
        ...
        if ($_GET['policy_codes']) {
            $last = str_split(str_replace(' ', '', $_GET['policy_codes']));
        } else {
            // 取号码
...
//        echo $this->response->setJsonContent($bet_arr)->getContent();
    ...
    public function getLocalPolicyAction()
    {
        try {
            ...
            $bet_times = [1, 2, 4, 8, 15, 30, 52, 90, 160, 280, 480, 820, 1400, 2400];
//            $bet_times = [2, 5, 10, 20, 50, 100, 200, 500, 1000, 2400];
            $unit = 1;
...
//        $unit = 2;
//        // $bet_times = [1, 2, 4, 8, 15, 30, 52, 90, 160, 280, 480, 820, 1400, 2400];
//        $bet_times = [2, 5, 10, 20, 40, 80, 150, 300, 520, 900, 1600, 2800, 4800, 8200, 14000, 24000];
//        $codes = implode('', $this->getPolicyBetCodes(40, 0, $code_count));
//        $this->renderJson([
//            'status' => Conster::AJAX_SUCCESS,
//            'codes' => $codes,
//            'period' => $period,
...
//        ]);
    ...
    public function makeBetAction()
    {
        ...
        $error_count = 0;
        $last_is_win = false;
        for ($i = 480 * 4; $i > 480 * 3;  $i--) {
            $win_bet = Lottery::findFirst(['order' => 'period desc', 'offset' => $i]);

            // 没有号码，或者错误3次时换号码 or 中了就换号码
//            if ($last_code == null || $last_is_win) {
//                $last_code = $this->getPolicyBetCodes($limit, $i + 1, $code_count);
//            } else if ($error_count > 0) {
//                $last_code = array_diff(range(0, 9), $last_code);
////                $error_count = 0;
//            }
            $last_code = $this->getPolicyBetCodes($limit, $i + 1, $code_count);

            $last_is_win = $is_win = in_array($win_bet->code5, $last_code);
            if ($is_win) {
                $error_count = 0;
            } else {
                $error_count++;
    ...
    public function sendLastCodeAction()
    {
        try {
            ...
            // 组三连续4次自动预警
            $lotteries = Lottery::find(['order' => 'period desc', 'limit' => 4]);
            $is_group3_before = true;
            $is_group3_after = true;
            $send_codes = '';
            foreach ($lotteries as $lottery) {
                if ($lottery->group3_before == 0)
                    $is_group3_before = false;
                if ($lottery->group3_after == 0)
                    $is_group3_after = false;

                $send_codes .= $lottery->period . ':  ' . $lottery->code1 . $lottery->code2 . $lottery->code3 . $lottery->code4 . $lottery->code5 . "\n";
            }
            if ($is_group3_before) {
                $this->sendDing('前三组三连续4次' . "\n" . $send_codes);
            }
            if ($is_group3_after) {
                $this->sendDing('后三组三连续4次' . "\n" . $send_codes);
            }
    ...
    private function sendDing($msg) {
        try {
            $ding = new DingTalk();
            $result = $ding->sendText("manager583", $msg);
    ...
    private function saveLastCode($data)
    {
        ...
        foreach ($data as $item) {
            ...
            if (Lottery::count("period = '{$item['period']}'") == 0) {
                // 组三判断
                $before = array_unique([$item['code'][0], $item['code'][1], $item['code'][2]]);
                $after = array_unique([$item['code'][2], $item['code'][3], $item['code'][4]]);
                ...
                $lottery->group3_before = count($before) < 3 ? 1 : 0; // 前三组三
                $lottery->group3_after = count($after) < 3 ? 1 : 0; // 后三组三
                $lottery->save();
                $count++;
            }
