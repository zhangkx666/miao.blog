...
class Markdown
{
    ...
    private function parseCode(array $lines, array $parts)
    {
        ...
        // data-start="1" class="line-numbers" . 代码高亮用 by @robbie
        return preg_match("/^\s*$/", $str) ? '' :
            '<pre data-start="1" class="line-numbers"><code' . (!empty($lang) ? " class=\"{$lang}\"" : '')
            . (!empty($rel) ? " rel=\"{$rel}\"" : '') . '>'
     ...
     * @return string
     */
    private function getFaceUrl($alt)
    {
        $alts = ["微笑", "嘻嘻", "哈哈", "可爱", "可怜", "挖鼻", "吃惊", "害羞", "挤眼", "闭嘴", "鄙视", "爱你", "泪", "偷笑",
        ...
        $index = array_search($alt, $alts);
        return $index !== false ? "/public/layui/images/face/$index.gif" : '';
    }
}
